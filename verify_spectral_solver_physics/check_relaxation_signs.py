...
shear_source_expected = 2.0 * coeffs.shear_viscosity * sigma[ix, iy, iz, 1, 1] / coeffs.shear_relaxation_time

print(f"Expected source terms (first-order only):")
...
if bulk_match:
    ...
    print(f"  ✗ Bulk relaxation sign error: {abs((dPi_dt[ix, iy, iz] - expected_total_bulk) / expected_total_bulk) * 100:.2f}%")

if shear_match:
    print("  ✓ Shear relaxation has correct signs")
else:
    print(f"  ✗ Shear relaxation sign error: {abs((dpi_dt[ix, iy, iz, 1, 1] - expected_total_shear) / expected_total_shear) * 100:.2f}%")
