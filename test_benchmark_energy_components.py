...
"""
Track energy components during benchmark evolution.

Uses the same transport coefficients as the sound wave benchmark to see
how energy is redistributed between perfect fluid, bulk, and shear contributions.
"""
...
print("=" * 80)
print("BENCHMARK ENERGY COMPONENT TRACKING")
print("=" * 80)
print()
...
print("=" * 80)
print(f"Initial:")
print(f"  |E_Π/E_tot| = {abs(data_0['E_bulk']/data_0['E_total'])*100:.8f}%")
print(f"  |E_π/E_tot| = {abs(data_0['E_shear']/data_0['E_total'])*100:.8f}%")
print()
print(f"Peak:")
...
print("Viscous effects in sound wave propagation:")
print()
print(f"1. Bulk viscosity (ζ={coeffs.bulk_viscosity}, τ_Π={coeffs.bulk_relaxation_time}):")
print(f"   - Resists compression/expansion in wave")
print(f"   - Peak contribution: {abs(max_E_bulk/data_0['E_total'])*100:.6f}% of total energy")
print(f"   - Π_rms grows from {data_0['Pi_rms']:.2e} to peak {max_Pi_rms:.2e}")
print()

print(f"2. Shear viscosity (η={coeffs.shear_viscosity}, τ_π={coeffs.shear_relaxation_time}):")
print(f"   - Dissipates velocity gradients")
print(f"   - Peak contribution: {abs(max_E_shear/data_0['E_total'])*100:.6f}% of total energy")
print(f"   - π_rms grows from {data_0['pi_rms']:.2e} to peak {max_pi_rms:.2e}")
print()
...
if abs(dE_total/data_0['E_total']) < 0.01:  # 0.01% = 1e-4
    ...
    print(f"  ✓ Viscous effects redistribute energy but don't create/destroy it")
else:
...
print("=" * 80)
print()
print(f"With benchmark parameters (η={coeffs.shear_viscosity}, ζ={coeffs.bulk_viscosity}):")
print(f"  • Viscous energy contributions are ~{max(abs(max_E_bulk), abs(max_E_shear))/data_0['E_total']*100:.6f}% of total")
print(f"  • Perfect fluid energy dominates (>99.99%)")
print(f"  • Energy redistribution is reversible (oscillates with wave)")
print(f"  • Total energy conserved to {abs(dE_total/data_0['E_total'])*100:.4f}%")
print()
print("✓ All components correctly computed with benchmark coefficients")
print()
print("=" * 80)
