...
class CovariantDerivative:
    ...
    def divergence(
        ...
        if tensor_field.rank == 1:
            ...
            divergence = np.zeros(4) if isinstance(tensor_field.components, np.ndarray) else sp.zeros(4, 1)
...
class ProjectionOperator:
    ...
    def __init__(self, four_velocity: FourVector, metric: 'MetricBase'):
        ...
        if signature[0] < 0:  # Mostly-plus signature (-,+,+,+)
            ...
            if abs(mag_sq - expected_norm) > 1e-10:
                warnings.warn(f"Four-velocity should be normalized: u^μ u_μ = -1, got {mag_sq:.2e}", stacklevel=2)
    ...
    def project_vector_perpendicular(self, vector: FourVector) -> FourVector:
        ...
        if isinstance(vector.components, np.ndarray):
            perp_components = optimized_einsum('mn,n->m', delta.components, vector_lowered.components)
        else:
    ...
    def project_tensor_spatial(self, tensor: TensorField) -> TensorField:
        ...
        if isinstance(tensor.components, np.ndarray):
            spatial = optimized_einsum('ma,nb,ab->mn',
                                      delta.components, delta.components, tensor_lowered.components)
        else:
    ...
    def extract_scalar_density(self, tensor: TensorField) -> float | sp.Expr:
        ...
        if isinstance(tensor.components, np.ndarray):
            scalar = optimized_einsum('i,j,ij->', u_cov.components, u_cov.components, tensor.components)
        else:
            scalar = 0
            for mu in range(4):
                for nu in range(4):
                    scalar += u_cov.components[mu] * u_cov.components[nu] * tensor.components[mu, nu]
    ...
    def extract_vector_density(self, tensor: TensorField) -> FourVector:
        ...
        if isinstance(tensor.components, np.ndarray):
            vector = -optimized_einsum('mn,a,an->m',
                                      delta.components, u_cov.components, tensor_mixed.components)
        else:
            vector = sp.zeros(4, 1)
            for mu in range(4):
                for nu in range(4):
                    for alpha in range(4):
                        vector[mu] -= (delta.components[mu, nu] * u_cov.components[alpha] *
                                     tensor_mixed.components[alpha, nu])

        return FourVector(vector, False, self.metric)

    def decompose_tensor(self, tensor: TensorField) -> dict[str, float | sp.Expr | FourVector | TensorField]:
        """
