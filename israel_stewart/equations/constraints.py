...
class ThermodynamicConstraints:
    ...
    def check_causality_constraints(
        ...
        # Speed of sound
        cs_squared = self._compute_speed_of_sound_squared(
        ...
        if coefficients.shear_viscosity and coefficients.shear_relaxation_time:
            ...
            v_shear_squared = (4 * eta / 3) / (
                entropy_density * temperature * tau_pi + 1e-12
            )
        ...
        result = {
            ...
            'max_speed': max(
                np.max(speed) for speed in causality_speeds.values()
                if isinstance(speed, np.ndarray) and speed.size > 0
            ) if causality_speeds else 0.0,
        }
    ...
    def check_stability_constraints(
        ...
        if coefficients.shear_viscosity is not None:
            if coefficients.shear_viscosity < 0:
                violations.append({
                    ...
                    'description': f'Negative shear viscosity: � = {coefficients.shear_viscosity:.6e}',
                    'severity': 'critical',
                })
        ...
        if coefficients.bulk_viscosity is not None:
            if coefficients.bulk_viscosity < 0:
                violations.append({
                    ...
                    'description': f'Negative bulk viscosity: � = {coefficients.bulk_viscosity:.6e}',
                    'severity': 'critical',
                })
        ...
        if coefficients.shear_relaxation_time is not None:
            if coefficients.shear_relaxation_time <= 0:
                violations.append({
                    ...
                    'description': f'Non-positive shear relaxation time: �� = {coefficients.shear_relaxation_time:.6e}',
                    'severity': 'critical',
                })
        ...
        if np.any(energy_density <= 0):
            ...
            violations.append({
                ...
                'description': f'Non-positive energy density: min(�) = {min_energy:.6e}',
                'severity': 'critical',
            })
        ...
        if np.any(entropy_production < -self.tolerance):
            ...
            violations.append({
                ...
                'description': f'Negative entropy production: min(�) = {min_entropy_prod:.6e}',
                'severity': 'warning',
            })
    ...
    def check_energy_conditions(
        ...
        Includes:
        - Weak Energy Condition (WEC): � e 0, � + P e 0
        - Null Energy Condition (NEC): � + P e 0
        - Strong Energy Condition (SEC): � + 3P e 0
        - Dominant Energy Condition (DEC): � e |P|
        """
        energy_density = fields.rho
        pressure = fields.pressure
        ...
        # Weak Energy Condition: � e 0 and � + P e 0
        ...
        if wec1_violation:
            ...
            violations.append({
                ...
                'description': f'WEC violated: negative energy density, min(�) = {min_rho:.6e}',
                'severity': 'critical',
            })
        ...
        if wec2_violation:
            ...
            violations.append({
                ...
                'description': f'WEC violated: � + P < 0, min(�+P) = {min_rho_plus_p:.6e}',
                'severity': 'critical',
            })

        # Null Energy Condition: � + P e 0 (same as WEC2)
        nec_status = 'satisfied' if not wec2_violation else 'violated'

        # Strong Energy Condition: � + 3P e 0
        ...
        if sec_violation:
            ...
            violations.append({
                'type': 'sec_violation',
                'description': f'SEC violated: � + 3P < 0, min(�+3P) = {min_rho_plus_3p:.6e}',
                'severity': 'warning',
            })

        # Dominant Energy Condition: � e |P|
        ...
        if dec_violation:
            ...
            violations.append({
                'type': 'dec_violation',
                'description': f'DEC violated: � < |P|, max violation = {max_violation:.6e}',
                'severity': 'warning',
            })
        ...
        return {
            ...
            'individual_conditions': {
                'weak_energy_condition': 'satisfied' if not (wec1_violation or wec2_violation) else 'violated',
    ...
    def check_transport_coefficient_bounds(
        self,
        coefficients: TransportCoefficients,
        fields: Optional[ISFieldConfiguration] = None,
    ) -> Dict[str, Any]:
        """
        ...
        if coefficients.shear_viscosity and fields:
            ...
            if np.any(eta_over_s < kss_bound - self.tolerance):
                ...
                violations.append({
                    ...
                    'description': f'�/s below KSS bound: min(�/s) = {min_eta_s:.6f} < {kss_bound:.6f}',
                    'severity': 'warning',
                })
        ...
        if coefficients.shear_relaxation_time and coefficients.shear_viscosity and fields:
            ...
            if np.any(tau_ratio < 0.01) or np.any(tau_ratio > 100.0):
                violations.append({
                    ...
                    'description': f'Relaxation time far from natural scale: ��/(�/sT) ~ {np.mean(tau_ratio):.2f}',
                    'severity': 'info',
                })
        ...
        result = {
            ...
            'overall_status': 'reasonable' if not any(v['severity'] in ['critical', 'warning'] for v in violations) else 'questionable',
        }
    ...
    def _check_second_order_bounds(self, coefficients: TransportCoefficients) -> Dict[str, Any]:
        ...
        # ��� bounds (self-interaction of shear stress)
        ...
        if lambda_pi_pi is not None:
            ...
            if lambda_pi_pi < 0 or lambda_pi_pi > 5.0:
                violations.append({
                    ...
                    'description': f'��� outside expected range: ��� = {lambda_pi_pi:.3f}',
                    'severity': 'info',
                })

        # � coefficients (bulk pressure nonlinearities)
    ...
    def enforce_constraints(
        ...
        constrained_fields.temperature = temp_constrained.astype(np.float64).reshape(constrained_fields.temperature.shape)

        rho_constrained = np.maximum(constrained_fields.rho, self.tolerance)
        constrained_fields.rho = rho_constrained.astype(np.float64).reshape(constrained_fields.rho.shape)
    ...
    def _compute_speed_of_sound_squared(
        ...
        if self.eos == "ideal":
            # For ideal gas: cs� = P/� = 1/3
            return np.full_like(temperature, 1.0 / 3.0)
        else:
            # General case: cs� = P/� (finite difference approximation)
