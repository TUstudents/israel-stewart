...
class TestTransportCoefficientsEnhanced:
    ...
    def test_second_order_initialization(self):
    ...
    def test_stability_constraints(self):
        ...
        with pytest.raises(ValueError, match="Shear viscosity must be non-negative"):
            TransportCoefficients(shear_viscosity=-0.1)

        with pytest.raises(ValueError, match="Bulk viscosity must be non-negative"):
            TransportCoefficients(shear_viscosity=0.1, bulk_viscosity=-0.05)

        # Invalid relaxation times
        with pytest.raises(ValueError, match="shear_relaxation_time must be positive"):
            TransportCoefficients(shear_viscosity=0.1, shear_relaxation_time=-0.1)

    def test_large_coupling_warning(self):
...
class TestISFieldConfigurationEnhanced:
    ...
    def test_dissipative_vector_methods(self, setup_field_config):
        """Test dissipative vector packing/unpacking."""
        config = setup_field_config

        # Set some test dissipative fields
        config.Pi = np.random.rand(*config.grid.shape)
        config.pi_munu = np.random.rand(*config.grid.shape, 4, 4)
        config.q_mu = np.random.rand(*config.grid.shape, 4)
    ...
    def test_dissipative_field_count(self, setup_field_config):
    ...
    def test_dissipative_vector_size_validation(self, setup_field_config):
...
class TestISRelaxationEquations:
    ...
    @pytest.fixture
    def setup_relaxation_system(self):
        ...
        return relaxation, fields, grid

    def test_relaxation_initialization(self, setup_relaxation_system):
    ...
    def test_bulk_rhs_physics(self, setup_relaxation_system):
    ...
    def test_shear_rhs_physics(self, setup_relaxation_system):
    ...
    def test_explicit_evolution(self, setup_relaxation_system):
    ...
    def test_implicit_evolution(self, setup_relaxation_system):
    ...
    def test_exponential_integrator(self, setup_relaxation_system):
    ...
    def test_stability_analysis(self, setup_relaxation_system):
...
class TestRelaxationPhysics:
    ...
    def test_second_order_coupling_effects(self):
        ...
        assert not np.allclose(fields1.Pi, fields2.Pi, rtol=0.1)
        assert not np.allclose(fields1.pi_munu, fields2.pi_munu, rtol=0.1)

    def test_milne_coordinates(self):
