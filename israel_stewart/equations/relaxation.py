...
from ..core.fields import TransportCoefficients, ISFieldConfiguration
from ..core.spacetime_grid import SpacetimeGrid
from ..core.tensors import TensorField
...
class ISRelaxationEquations:
    ...
    def _build_symbolic_equations(self) -> Dict[str, sp.Expr]:
        ...
        pi_11, pi_12, pi_13, pi_22, pi_23, pi_33 = sp.symbols('pi_11 pi_12 pi_13 pi_22 pi_23 pi_33', real=True)
        ...
        bulk_nonlinear = (xi_1 * Pi * theta +
                         xi_2 * Pi**2 / (zeta * tau_Pi) +
                         lambda_Pi_pi * pi_00 * theta)  # Simplified shear-bulk coupling
        ...
        shear_nonlinear = (-tau_pi_pi * pi_00**2 / (eta * tau_pi) +
        ...
        heat_nonlinear = (lambda_q_pi * pi_00 * sp.Symbol('nabla_0_T') -
                         tau_q * q_0 * theta)
    ...
    def compute_relaxation_rhs(self, fields: ISFieldConfiguration) -> np.ndarray:
        ...
        dpi_munu_dt = self._shear_rhs(pi_munu, Pi, q_mu, expansion_scalar,
                                     shear_tensor, vorticity_tensor, temp_gradient)
        dq_mu_dt = self._heat_rhs(q_mu, pi_munu, expansion_scalar, temp_gradient)

        # Pack into dissipative vector format
        return np.concatenate([
            dPi_dt.flatten(),
            dpi_munu_dt.reshape(-1),
            dq_mu_dt.reshape(-1)
        ])

    def _bulk_rhs(self, Pi: np.ndarray, pi_munu: np.ndarray,
                  theta: np.ndarray) -> np.ndarray:
        """Compute bulk pressure evolution."""
        # Linear relaxation term
        linear = -Pi / self.coeffs.bulk_relaxation_time if self.coeffs.bulk_relaxation_time else np.zeros_like(Pi)
        ...
        if self.coeffs.xi_2 != 0 and self.coeffs.bulk_viscosity > 0 and self.coeffs.bulk_relaxation_time:
            nonlinear += (self.coeffs.xi_2 * Pi**2 /
    ...
    def _shear_rhs(self, pi_munu: np.ndarray, Pi: np.ndarray, q_mu: np.ndarray,
                   theta: np.ndarray, sigma_munu: np.ndarray,
                   omega_munu: np.ndarray, nabla_T: np.ndarray) -> np.ndarray:
        """Compute shear tensor evolution."""
        # Linear relaxation
        linear = -pi_munu / self.coeffs.shear_relaxation_time if self.coeffs.shear_relaxation_time else np.zeros_like(pi_munu)
        ...
        # Expansion coupling
        if self.coeffs.lambda_pi_pi != 0:
            expansion_term = self.coeffs.lambda_pi_pi * np.einsum('...ij,...->...ij',
                                                                  pi_munu, theta)
            ...
            for mu in range(4):
                for nu in range(4):
                    if nabla_T.shape[-1] > max(mu, nu):
                        heat_term = (self.coeffs.lambda_pi_q *
                                    (q_mu[..., mu] * nabla_T[..., nu] +
                                     q_mu[..., nu] * nabla_T[..., mu]) / 2)
        ...
        if self.coeffs.tau_pi_pi != 0 and self.coeffs.shear_relaxation_time:
            ...
            pi_pi_term = -self.coeffs.tau_pi_pi * np.einsum('...ik,...kj->...ij',
                                                           pi_munu, pi_munu)
    ...
    def _heat_rhs(self, q_mu: np.ndarray, pi_munu: np.ndarray,
                  theta: np.ndarray, nabla_T: np.ndarray) -> np.ndarray:
        """Compute heat flux evolution."""
        # Linear relaxation
        linear = -q_mu / self.coeffs.heat_relaxation_time if self.coeffs.heat_relaxation_time else np.zeros_like(q_mu)
    ...
    def evolve_relaxation(self, fields: ISFieldConfiguration,
                         dt: float,
                         method: str = 'explicit') -> None:
        """
    ...
    def _implicit_evolution(self, fields: ISFieldConfiguration, dt: float) -> None:
        ...
        try:
            ...
            x_solution = newton_krylov(residual, x_initial,
                                     method='gmres',
                                     f_tol=1e-8,
                                     maxiter=50)
            ...
            warnings.warn(f"Implicit solver failed: {e}. Using explicit step.",
                         stacklevel=3)
            self._explicit_evolution(fields, dt)
