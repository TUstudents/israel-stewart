...
from ..core.computational_optimization import (
    cached_operation,
...
class SpectralISolver:
    ...
    # ========== Phase 3: Computational Optimization Methods ==========

    @profile_operation("phase3_optimized_fft")
    def phase3_optimized_fft(self, field: np.ndarray, forward: bool = True,
                            real_fft: bool = False) -> np.ndarray:
        """
        Phase 3 optimized FFT with advanced plan caching and FFTW backend.

        Uses computational optimization infrastructure for maximum performance
        with intelligent plan caching and backend selection.

        Args:
            field: Input field for FFT transformation
            forward: If True, forward FFT; if False, inverse FFT
            real_fft: If True, use real FFT for real-valued inputs

        Returns:
            Transformed field with optimal performance
        """
        with profile_operation("fft_optimization", {"shape": field.shape, "real_fft": real_fft}):
            return optimized_fft(field, forward=forward, real_fft=real_fft)

    @profile_operation("phase3_cached_divergence")
    def phase3_cached_divergence(self, vector_field: np.ndarray) -> np.ndarray:
        """
        Phase 3 cached divergence computation with intelligent result caching.

        Caches divergence results for repeated operations on similar fields,
        significantly reducing computational overhead.

        Args:
            vector_field: Vector field to compute divergence (..., 3)

        Returns:
            Scalar divergence field
        """
        # Create cache key from field properties
        field_key = f"div_{vector_field.shape}_{self.dx}_{self.dy}_{self.dz}"
        ...
        return cached_operation(field_key, compute_divergence)

    @profile_operation("phase3_fused_spectral_derivative")
    def phase3_fused_spectral_derivative(self, field: np.ndarray,
                                       axis: int = 0) -> np.ndarray:
        """
        Phase 3 fused spectral derivative with operation fusion.

        Combines FFT, derivative multiplication, and inverse FFT into
        a single optimized operation to minimize memory transfers.

        Args:
            field: Input field for derivative computation
            axis: Axis along which to compute derivative (0=x, 1=y, 2=z)

        Returns:
            Spectral derivative along specified axis
        """
        def compute_fused_derivative():
            # Forward FFT with caching
            field_k = self.phase3_optimized_fft(field, forward=True, real_fft=True)

            # Get wavenumber for specified axis
            if axis == 0:
                kx = 2 * np.pi * np.fft.fftfreq(self.nx, self.dx)
                k_axis = kx[np.newaxis, :, np.newaxis]
            elif axis == 1:
                ky = 2 * np.pi * np.fft.fftfreq(self.ny, self.dy)
                k_axis = ky[np.newaxis, np.newaxis, :]
            elif axis == 2:
                kz = 2 * np.pi * np.fft.rfftfreq(self.nz, self.dz)
                k_axis = kz[np.newaxis, np.newaxis, :]
            else:
                raise ValueError(f"Invalid axis {axis}, must be 0, 1, or 2")

            # Vectorized derivative multiplication with broadcasting
            derivative_k = vectorized_operation(
                "fused_multiply_add",
                1j * k_axis, field_k, np.zeros_like(field_k)
            )

            # Inverse FFT with caching
            return self.phase3_optimized_fft(derivative_k, forward=False, real_fft=True)

        cache_key = f"spectral_deriv_{field.shape}_{axis}"
        return cached_operation(cache_key, compute_fused_derivative)

    @profile_operation("phase3_vectorized_tensor_contraction")
    def phase3_vectorized_tensor_contraction(self, tensor_a: np.ndarray,
                                           tensor_b: np.ndarray,
                                           axes: tuple[int, ...]) -> np.ndarray:
        """
        Phase 3 optimized tensor contraction with advanced vectorization.

        Uses optimal contraction paths and vectorized operations for
        maximum performance in tensor algebra.

        Args:
            tensor_a: First tensor for contraction
            tensor_b: Second tensor for contraction
            axes: Axes to contract over

        Returns:
            Contracted tensor result
        """
        contraction_key = f"contract_{tensor_a.shape}_{tensor_b.shape}_{axes}"
        ...
        return cached_operation(contraction_key, compute_contraction)

    @profile_operation("phase3_bulk_fft_transform")
    def phase3_bulk_fft_transform(self, fields_dict: dict[str, np.ndarray],
                                 forward: bool = True) -> dict[str, np.ndarray]:
        """
        Phase 3 bulk FFT transformation for multiple fields.

        Efficiently transforms multiple fields using shared plans and
        workspace management for optimal memory usage.

        Args:
            fields_dict: Dictionary of field name -> field array
            forward: Direction of FFT transformation
        ...
        # Group fields by shape for optimal plan reuse
        shape_groups = {}
        for name, field in fields_dict.items():
            shape = field.shape
            if shape not in shape_groups:
                shape_groups[shape] = []
            shape_groups[shape].append((name, field))

        # Transform each shape group with optimized plans
        for shape, field_list in shape_groups.items():
            # Get optimized FFT function for this shape
            optimizer = get_global_optimizer()
            with optimizer.optimized_fft_context() as fft_cache:
                # Determine if we can use real FFT
                real_fft = all(np.isrealobj(field) for _, field in field_list)
                fft_func = fft_cache.get_fft_function(
                    shape, np.float64 if real_fft else np.complex128,
                    forward, real_fft
                )

                # Transform all fields with same shape
                for name, field in field_list:
                    transformed_fields[name] = fft_func(field)

        return transformed_fields

    def get_phase3_performance_stats(self) -> dict[str, Any]:
        """Get Phase 3 computational optimization performance statistics."""
        optimizer = get_global_optimizer()
